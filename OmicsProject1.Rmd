...
fulldata  %>% summary(patient.SCORAD.index.SCORAD.SCORAD.Score..SCORAD_Score.)
...
# try some clustering
deucl_norm <- dist(t(omicsdata))
# write those into file to sqve computing time next time
write(x = deucl_norm, file = "expr_distance.txt")

plot(deucl_norm)
plot(flashClust::hclust(deucl_norm, method = "ward"))


# build annotation dataset
annotations <- data.frame(sample_id = colnames(omicsdata), MAARS_identifier = gsub('.{3}$', '', sample_id))
annotations <- dplyr::left_join(annotations, alldata, by = "sample_id") %>% select(sample_id, MAARS_identifier.x, clinical_group, lesional, CUSTOM_Age, Gender) %>% rename(MAARS_identifier = MAARS_identifier.x)
